...
| lisResultSourcedId | ?
| resourceLinkId | ?
| endpoint | The URL of the Materia widget
...
| Property | Description |
|-
| lisResultSourcedId | Represents a unique use of this resource in the Obojobo Module for the current visit
| resourceLinkId | Represents a unique use of this resource in the Obojobo Module, corresponding to the [LTI-1p0 standard](http://www.imsglobal.org/specs/ltiv1p1p1/implementation-guide)
...
> The `resourceLinkId` uniquely identifies a Materia widget inside an Obojobo Module. Each play of any given Materia widget will share the same `resourceLinkId`. The `resourceLinkId` may change when a Module is changed. Multiple modules may share the same `resourceLinkId` and should not be used to uniquely identify a Materia widget across multiple Obojobo modules. Additionally, interactions with the same Obojobo Module across different uses (as in, embedded in different courses) will share the same `resourceLinkId`.

> The `lisResultSourceId` uniquely identifies the interaction of a Materia widget inside an Obojobo module for a given visit. Therefore this ID can be used to find similar events for a given Materia widget inside an Obojobo module for the same visit. Additionally, this ID will be different for interactions for the same widget across multiple Obojobo modules as well as interactions for the same Obojobo module across different uses (as in, embedded in different courses).
